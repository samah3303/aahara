...
// In-Memory Session Storage (Simple Map for Demo)
    ...
    try {
        ...
        if (message && message.type === 'text') {
            ...
            if (decision.intent === 'CREATE_BOOKING') {
                // Assume Center 1 for demo simplicity (or ask for it)
            ...
            // 2. Service Selected -> Ask for Details
            ...
            // 3. Details Provided -> Ask next or Time?
            else if (decision.intent === 'INFORM_DETAILS') {
                if (session.data.expecting === 'product') {
                    session.data.product_type = decision.product_type || decision.quantity; // Fallback
                    reply = "Got it. And what is the total quantity? (e.g. 100 kg)";
                    session.data.expecting = 'quantity';
                    // Stay in SELECT_DETAILS
                } else {
                    session.data.quantity = decision.quantity || decision.product_type; // Fallback
